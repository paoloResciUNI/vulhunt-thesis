...
impl<'a> PlatformApi<'a> for WindowsBinary {
    fn should_check(
        arch: &[CheckerArch],
        data: &CheckScopeProjectData,
        attrs: &PlatformAttributes,
    ) -> Result<bool, CheckerError> {
        if let Some(larch) = attrs.get_attr::<WindowsComponentArch>() {
        ...
        let Some(actual_name) = attrs.get_attr::<WindowsComponentName>() else {
            return Ok(false);
        };
        ...
        if let Some(name) = data.name() {
            let matches = |name: &str| -> bool {
                name == &**actual_name
                    || matches!(attrs.get_attr::<WindowsComponentLinkedPaths>(), Some(links) if links.contains_name(name))
            };
        ...
        if let Some(prefix) = data.name_with_prefix() {
            let matches_prefix = |prefix: &str| -> bool {
                actual_name.starts_with(prefix)
                    || matches!(attrs.get_attr::<WindowsComponentLinkedPaths>(), Some(links) if links.names().any(|n| n.starts_with(prefix)))
            };
    ...
    fn configure_decompiler<'d>(
        ...
        if let Some(non_returning_functions) =
            project.try_get_analysis::<PropagatedWindowsNonReturningExternals>()
        {
