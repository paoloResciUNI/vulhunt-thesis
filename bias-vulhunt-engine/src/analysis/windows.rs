...
pub struct VulHuntWindowsAnalyser {
    config: VulHuntWindowsAnalyserConfig,
    checkers: CheckerDB,
    analyses: WindowsBinaryAnalysis,
    symbols: FLIRTSymbolManager,
    types: TypeManager,
}

#[derive(Debug, Error)]
pub enum VulhuntWindowsAnalyserError {
    #[error("cannot initialise type manager: {0}")]
    Types(#[from] TypeManagerError),
}

#[derive(Debug, Clone, Default)]
pub struct VulHuntWindowsAnalyserConfig {
    pub render: bool,
    pub ignore_errors: bool,
    pub module_directory: VulHuntModuleDir,
}

impl VulHuntWindowsAnalyserConfig {
    pub fn new() -> Self {
        Self::default()
    }
...
impl AnalysisGroupAnalyserForCode for VulHuntWindowsAnalyser {
    ...
    fn analyse_and_check(
        ...
        let mut engine = self.checkers.engine::<WindowsBinary>(
            &component,
            &project,
            self.symbols.clone(),
            self.types.clone(),
    ...
    fn analyse_and_prefilter(
        ...
        for checker in self.checkers.iter() {
            if checker.platform() == WindowsBinary::NAME
